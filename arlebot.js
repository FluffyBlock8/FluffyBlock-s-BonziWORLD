const PREFIX = "-";
const NAME = `Arle Nadja [${PREFIX}help]`;
const HELP =  `-
__^^Arle Nadja^^\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0__
\u00a0\u2022 ${PREFIX}help [cmd?]
\u00a0\u2022 ${PREFIX}source
...
setTimeout(() => { arle.emit("command", { list: ["color", "arle"] }) }, 5000);
setTimeout(() => { arle.emit("command", { list: ["speed", "62"] }) }, 5000);
setTimeout(() => { arle.emit("command", { list: ["pitch", "50"] }) }, 5000);
setTimeout(() => { send(`Arle Nadja is online. Type ${PREFIX}help to see commands.`); }, 10000);
...
let cmd = {
    ...
    say({ text, reply }) {
        if (text === "") {
            send("Puyo Puyo: You Just Got Dominated! Retard.");
        } else {
            send(text);
        }
    },
    	joke() {
		arle.emit("command", { list: ["joke"] })
        },
        	fact() {
		arle.emit("command", { list: ["fact"] })
        },
        	triggered() {
		arle.emit("command", { list: ["triggered"] })
        },
            	linux() {
		arle.emit("command", { list: ["linux"] })
        },
        	color() {
...
let hats_ = ["bucket", "troll", "bieber", "kamala", "maga", "evil", "bfdi", "elon", "tophat", "witch", "wizard", "horse", "bowtie", "obama", "cigar", "dank", "illuminati", "bear", "truck", "king"]
            ...
            try {
                ...
                for (curCmd of pipeline.slice(1)) {
                    ...
                    if (!Object.hasOwn(imgFilters, command)) {
                        throw Object.hasOwn(cmd, command)
...
arle.on("updateAll", (data) => {
    for (let [id, user] of Object.entries(data)) {
        userMap.set(id, user);
    }
});

arle.on("update", ({ guid, userPublic }) => userMap.set(guid, userPublic));

arle.on("leave", ({ guid }) => userMap.delete(guid));
